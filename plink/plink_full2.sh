...
#SBATCH --output=./full2_log.%j.out
#SBATCH --error=./full2_log.%j.err
...
export PLINK_RESULT_PATH="$PLINK_PATH/results/bac_age2"
...
if $plink_execute; then
  ...
  $PLINK_NEW_BIN --bfile $PLINK_OUTPUT_PATH/converted_genotyped --linear --adjust --pheno $PLINK_PATH/phenotype_full2.tsv --all-pheno --covar $PLINK_PATH/covariate_full.tsv --covar-number $covar_number --missing --out $PLINK_RESULT_PATH/result --noweb --allow-extra-chr --allow-no-sex \
