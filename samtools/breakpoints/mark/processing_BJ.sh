...
#SBATCH --mem=1G
#SBATCH --export=INPUT_PATH='/mnt/raid6/bacphagenetwork/data/02_samtools_analysis/Beijing',OUTPUT_PATH='/mnt/raid6/bacphagenetwork/data/03_samtools_marked/Beijing',STATS_PATH='/mnt/raid6/bacphagenetwork/data/03_samtools_marked_stats',INDEX_FILE='/mnt/raid6/bacphagenetwork/data/00_bwa_index/chm13v2/chm13v2.0_noY.fa'
...
samtools markdup -r -s $INPUT_PATH/${sample_name}.bam $OUTPUT_PATH/${sample_name}_marked.bam -f $STATS_PATH/${sample_name}.txt -O --reference $INDEX_FILE || { echo "Error: Failed to mark the duplicates of $INPUT_PATH/${sample_name}.bam"; exit 1; }
