...
def draw_histogram(static_file, output_file, sample_name):
    ...
    data_sorted['MapQ'] = data_sorted['MapQ'].astype(str)

    # Define the figure
    plt.figure(figsize=(10, 10), dpi=300)

    # Draw the histogram
    plt.bar(data_sorted['MapQ'], data_sorted['Count'])
    plt.xticks(rotation=90)
    plt.xlabel("MapQ")
    plt.ylabel("Count")
    plt.title("MapQ counts in " + sample_name)
    plt.tight_layout()
...
with open(tmp_file, "w") as outfile:
  for read in samfile.fetch():
        outfile.write(f"{read.mapping_quality}\n")
